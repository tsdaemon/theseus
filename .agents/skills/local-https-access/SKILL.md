...
description: Adds LAN-only HTTPS access (valid Let's Encrypt cert) for a Theseus service under the public domain, e.g. keycloak-admin.example.com. Use when a service needs a secure browser context (HTTPS) on the LAN, when adding a Traefik router on the web_secure entrypoint, or when a name under example.com must resolve only locally via Pi-hole.
---

# Local HTTPS access

Pattern: a hostname under `ansible_nas_domain` (e.g. `<svc>.example.com`) that resolves to the NAS only inside the LAN (Pi-hole), is served by Traefik on `web_secure` (443) with a wildcard Let's Encrypt cert, and is never published through the Cloudflare Tunnel.
...
| `web_secure` | `:443` | LAN, HTTPS, `<svc>.example.com`, wildcard cert (DNS-01) |
...
- DNS-01 must talk to the DNS host of the zone. `example.com` is registered at Porkbun but its nameservers are Cloudflare, so use `traefik_dns_provider: cloudflare` with `CF_DNS_API_TOKEN` (token: Zone DNS Edit + Zone Read, limited to the `example.com` zone) in `traefik_environment_variables`. Porkbun keys fail with `CREATE_ERROR`.
...
- Cloudflare has a wildcard proxied record for `*.example.com`, so any name without a local answer resolves to Cloudflare (SSL error 525). Every LAN-only name must be in `pihole_local_hostnames`.
